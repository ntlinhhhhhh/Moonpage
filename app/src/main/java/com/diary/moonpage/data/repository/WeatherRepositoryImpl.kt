...
class WeatherRepositoryImpl @Inject constructor(
    ...
    override suspend fun getWeatherConditions(lat: Double, lon: Double, date: LocalDate): Result<List<String>> {
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                if (date == today && body.currentWeather != null) {
                    ...
                    if (current.temperature < 20) conditions.add("Cold")
                    if (current.windspeed > 25) conditions.add("Windy")
                } else if (body.daily != null && body.daily.time.isNotEmpty()) {
                    // Use daily summary for past dates (Archive) or day summary (Forecast)
                    ...
                    if (daily.temperatureMax[0] < 20) conditions.add("Cold")
                    if (daily.windspeedMax[0] > 25) conditions.add("Windy")
                }

                Result.success(conditions.distinct())
            } else {
